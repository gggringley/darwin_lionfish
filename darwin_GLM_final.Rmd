...
```{r}
pca.plot3 <- autoplot(d.factanal2, data = darwin, colour = 'lionfish_den', size = 'lionfish_den',
         loadings = TRUE, loadings.label = TRUE, loadings.label.size  = 5) + theme_classic() +  scale_color_continuous(limits=c(0, 700), breaks=seq(0, 700, by=200)) +
   guides(color= guide_legend(), size=guide_legend()) + scale_size_continuous(limits=c(0, 700), breaks=seq(0, 700, by=200)) + scale_size_continuous(range = c(2,12))
pca.plot3
```
```{r}
ggsave(filename = "lionfish_pca_rev3.eps", plot = pca.plot3, device = "eps", path = "~/Desktop/Lionfish")
```
